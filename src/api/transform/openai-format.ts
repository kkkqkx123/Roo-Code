...
/**
 * Type for reasoning detail elements from reasoning models.
 */
			...
			if (anthropicMessage.role === "user") {
				...
				toolMessages.forEach((toolMessage) => {
					...
					openAiMessages.push({
						...
						// Use "(empty)" placeholder for empty content to satisfy providers
				...
				const baseMessage: OpenAI.Chat.ChatCompletionAssistantMessageParam & {
					...
					role: "assistant",
					// Use empty string instead of undefined for providers that require
					// every message to have content in the "parts" field
					content: content ?? "",
				}
