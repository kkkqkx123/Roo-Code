...
/**
	* Concrete implementation of OpenAI-compatible provider for the "openai" provider type.
	* This class bridges the gap between ProviderSettings and OpenAICompatibleConfig.
	*/
export class OpenAICompatibleProviderImpl extends OpenAICompatibleHandler {
	constructor(options: ApiHandlerOptions) {
		// Validate required configuration
		if (!options.openAiBaseUrl) {
			throw new Error("OpenAI-compatible provider requires openAiBaseUrl to be configured")
		}
		if (!options.openAiApiKey) {
			throw new Error("OpenAI-compatible provider requires openAiApiKey to be configured")
		}
		if (!options.openAiModelId) {
			throw new Error("OpenAI-compatible provider requires openAiModelId to be configured")
		}
		if (!options.openAiCustomModelInfo) {
			throw new Error("OpenAI-compatible provider requires openAiCustomModelInfo to be configured")
		}
		...
		const config: OpenAICompatibleConfig = {
			...
			modelId: options.openAiModelId,
			modelInfo: options.openAiCustomModelInfo,
			headers: options.openAiHeaders,
	...
	override getModel(): { id: string; info: ModelInfo; maxTokens?: number; temperature?: number } {
		return {
			...
			temperature: this.options.modelTemperature ?? undefined,
		}
	}
}
