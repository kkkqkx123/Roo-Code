...
export class Terminal extends BaseTerminal {
	...
	constructor(id: number, terminal: vscode.Terminal | undefined, cwd: string) {
		...
		this.terminal = terminal ?? vscode.window.createTerminal({ cwd, name: "Coder", iconPath, env })

		if (Terminal.getTerminalZdotdir()) {
			const zdotdir = env.ZDOTDIR
			if (zdotdir) {
				ShellIntegrationManager.terminalTmpDirs.set(id, zdotdir)
			}
		}
	}
	...
	public static async getTerminalContents(commands = -1): Promise<string> {
		...
		try {
			...
			if (lastLine) {
				...
				while (i >= 0) {
					const line = lines[i]
					if (line && line.trim().startsWith(lastLine)) {
						break
					}
					i--
				}
