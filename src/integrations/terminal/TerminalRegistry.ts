...
export class TerminalRegistry {
	...
	private static getAllTerminals(): RooTerminal[] {
		...
		// Safety check: ensure all terminals are managed by Roo-Code
		// This prevents any external terminals from being used
		const managedTerminals = this.terminals.filter((t) => {
			// All terminals should have been created via createTerminal()
			// which ensures they are either Terminal or ExecaTerminal instances
			return t instanceof Terminal || t instanceof ExecaTerminal
		})

		// Log warning if any unmanaged terminals were found
		if (managedTerminals.length !== this.terminals.length) {
			console.warn(
				`[TerminalRegistry] Found ${this.terminals.length - managedTerminals.length} unmanaged terminal(s), removing from registry`,
			)
			this.terminals = managedTerminals
		}
