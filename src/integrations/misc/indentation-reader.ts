...
import {
	...
	DEFAULT_MAX_LEVELS,
	MAX_LINE_LENGTH,
} from "../../core/tools/schemas/read_file"
...
function computeIncludedRanges(lines: LineRecord[]): Array<[number, number]> {
	...
	let rangeStart = firstLine.lineNumber
	let rangeEnd = firstLine.lineNumber
...
export function readWithIndentation(content: string, options: IndentationReadOptions): IndentationReadResult {
	...
	const anchorIndent = effectiveIndents[anchorIdx] ?? 0
	...
	if (finalLimit === 1) {
		const anchorLineRecord = lines[anchorIdx]
		if (!anchorLineRecord) {
			return {
				content: "",
				includedRanges: [],
		...
		const singleLine = [anchorLineRecord]
		return {
	...
	const result: LineRecord[] = [anchorLineRecord]
	...
	while (result.length < finalLimit) {
		...
		// Expand upward
		const effectiveIndentUp = effectiveIndents[i]
		if (i >= 0 && effectiveIndentUp !== undefined && effectiveIndentUp >= minIndent) {
			const lineUp = lines[i]
			if (lineUp) {
				result.unshift(lineUp)
				progressed = true
			}

			// Handle sibling exclusion at min indent
			if (effectiveIndentUp === minIndent && !includeSiblings) {
				const lineForComment = lines[i]
				const allowHeader = includeHeader && lineForComment && isComment(lineForComment)
		...
		// Expand downward
		const effectiveIndentDown = effectiveIndents[j]
		if (j < lines.length && effectiveIndentDown !== undefined && effectiveIndentDown >= minIndent) {
			const lineDown = lines[j]
			if (lineDown) {
				result.push(lineDown)
				progressed = true
			}

			// Handle sibling exclusion at min indent
			if (effectiveIndentDown === minIndent && !includeSiblings) {
				if (jMinCount > 0) {
