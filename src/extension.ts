...
async function checkWorktreeAutoOpen(
	...
	try {
		...
		const currentPath = firstFolder.uri.fsPath
		...
		if (normalizePath(currentPath) === normalizePath(worktreeAutoOpenPath)) {
			...
			outputChannel.appendLine(`[Worktree] Auto-opening Coder sidebar for worktree: ${worktreeAutoOpenPath}`)

			// Open the Coder sidebar with a slight delay to ensure UI is ready
			setTimeout(async () => {
				try {
...
export async function activate(context: vscodeTypes.ExtensionContext) {
	...
	const mdmService = await MdmService.createInstance(createDualLogger(createOutputChannelLogger(outputChannel)))
	...
	// Implements the `CoderAPI` interface.
