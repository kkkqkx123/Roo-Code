...
export const defaultModeSlug = modes[0]?.slug || "code"
...
export function getModeBySlug(slug: string, customModes?: ModeConfig[]): ModeConfig | undefined {
	...
	return modes.find((mode: ModeConfig) => mode.slug === slug)
}
...
export async function getFullModeDetails(
	...
	if (!baseMode) {
		throw new Error(`No mode found for slug: ${modeSlug}`)
	}
