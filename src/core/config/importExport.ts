...
export async function importSettingsFromPath(
	...
	try {
		...
		if (currentApiConfigName && !validApiConfigs[currentApiConfigName]) {
...
export const importSettings = async ({ providerSettingsManager, contextProxy, customModesManager }: ImportOptions) => {
	...
	const defaultUri = resolveDefaultSaveUri(contextProxy, "lastSettingsExportPath", "coder-settings.json", {
		useWorkspace: false,
...
export const exportSettings = async ({ providerSettingsManager, contextProxy }: ExportOptions) => {
	const defaultUri = await resolveDefaultSaveUri(contextProxy, "lastSettingsExportPath", "coder-settings.json", {
		useWorkspace: false,
		fallbackDir: path.join(os.homedir(), "Downloads"),
	})
