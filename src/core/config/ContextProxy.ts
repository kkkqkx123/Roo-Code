...
type CoderSettingsKey = keyof CoderSettings
...
export class ContextProxy {
	 ...
	 */
	private sanitizeProviderValues(values: CoderSettings): CoderSettings {
		...
		for (const key of legacyKeys) {
			if (key in sanitizedValues) {
				...
				sanitizedValues = copy as CoderSettings
			}
		}
		...
		if (values.apiProvider !== undefined && !isKnownProvider) {
			...
			return restValues as CoderSettings
		}
	...
	public async setValue<K extends CoderSettingsKey>(key: K, value: CoderSettings[K]) {
	...
	public getValue<K extends CoderSettingsKey>(key: K): CoderSettings[K] {
		return isSecretStateKey(key)
			? (this.getSecret(key as SecretStateKey) as CoderSettings[K])
			: (this.getGlobalState(key as GlobalStateKey) as CoderSettings[K])
	}

	public getValues(): CoderSettings {
	...
	public async setValues(values: CoderSettings) {
		const entries = Object.entries(values) as [keyof CoderSettings, CoderSettings[keyof CoderSettings]][]
