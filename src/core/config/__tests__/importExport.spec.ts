...
import type { ProviderName } from "@coder/types"
...
describe("importExport", () => {
	...
	describe("importSettings", () => {
		...
		describe("lenient import with invalid providers", () => {
			...
			it("should skip completely invalid profiles and return warnings", async () => {
				...
				expect((result as { warnings?: string[] }).warnings).toBeDefined()
				expect((result as { warnings?: string[] }).warnings!.some((w: string) => w.includes("type-invalid"))).toBe(true)
				expect((result as { warnings?: string[] }).warnings!.some((w: string) => w.includes("skipped"))).toBe(true)
			...
			it("should handle multiple profiles with mixed valid and invalid providers", async () => {
				...
				expect(warnings.some((w: string) => w.includes("old-claude-profile"))).toBe(true)
				expect(warnings.some((w: string) => w.includes("another-invalid"))).toBe(true)
			...
			it("should fallback currentApiConfigName when the imported current profile was skipped", async () => {
				...
				expect(warnings.some((w: string) => w.includes("invalid-current-profile") && w.includes("skipped"))).toBe(true)
				expect(
					warnings.some(
						(w: string) =>
	...
	describe("exportSettings", () => {
		...
		it("should export settings to the selected file location", async () => {
			...
			expect(safeWriteJson).toHaveBeenCalledWith("/mock/path/coder-settings.json", {
		...
		it("should use the correct default save location", async () => {
			...
			expect(vscode.Uri.file).toHaveBeenCalledWith(path.join("/mock/home", "Downloads", "coder-settings.json"))
		})
		...
		it.each([
			...
			async ({ providerName, modelId, providerId }) => {
				...
				// Save a gemini provider config with token fields
				await realProviderSettingsManager.saveConfig(providerName, {
					apiProvider: "gemini" as ProviderName,
					apiModelId: modelId,
					id: providerId,
