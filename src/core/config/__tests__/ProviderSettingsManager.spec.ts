...
describe("ProviderSettingsManager", () => {
	...
	describe("SaveConfig", () => {
		...
		it("should preserve full fields including legacy provider-specific keys when saving retired provider profiles", async () => {
			...
			// Include a provider-specific field (openAiApiKey) that is
			// in the schema — passthrough() must keep it.
			const config = {
				apiProvider: "openai",
				apiKey: "test-key",
				apiModelId: "gpt-4",
			...
			await providerSettingsManager.saveConfig("test", config)

			const storedConfig = JSON.parse(mockSecrets.store.mock.calls[mockSecrets.store.mock.calls.length - 1]![1])
			expect(storedConfig.apiConfigs.test.apiProvider).toBe("openai")
			expect(storedConfig.apiConfigs.test.apiKey).toBe("test-key")
			expect(storedConfig.apiConfigs.test.apiModelId).toBe("gpt-4")
			expect(storedConfig.apiConfigs.test.openAiBaseUrl).toBe("https://api.openai.example/v1")
			expect(storedConfig.apiConfigs.test.openAiApiKey).toBe("test-openai-key")
			...
			expect(storedConfig.apiConfigs.retired.openAiApiKey).toBe("legacy-openai-specific-key")
	...
	describe("LoadConfig", () => {
		...
		it("should preserve retired providers and their fields including legacy provider-specific keys during initialize", async () => {
			...
			expect(storedConfig.apiConfigs.retiredProvider.apiProvider).toBe("openai")
			...
			// Verify legacy provider-specific field is preserved via passthrough
			expect(storedConfig.apiConfigs.retiredProvider.openAiApiKey).toBe("legacy-openai-key")
		})
	...
	describe("Export", () => {
		it("should preserve retired provider profiles with full fields", async () => {
			...
			expect(exported.apiConfigs.retired!.apiProvider).toBe("openai")
