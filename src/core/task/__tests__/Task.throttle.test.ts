import { CoderEventName, ProviderSettings, TokenUsage, ToolUsage } from "@coder/types"
...
describe("Task token usage throttling", () => {
	...
	test("should emit TaskTokenUsageUpdated immediately on first change", async () => {
		...
		expect(emitSpy).toHaveBeenCalledWith(
			CoderEventName.TaskTokenUsageUpdated,
			task.taskId,
			expect.any(Object),
			expect.any(Object),
		)
	...
	test("should throttle subsequent emissions within 2 seconds", async () => {
		...
		const firstEmitCount = emitSpy.mock.calls.filter(
			(call) => call[0] === CoderEventName.TaskTokenUsageUpdated,
		).length
	...
	test("should force final emission on task abort", async () => {
		...
		const tokenUsageUpdateIndex = calls.findIndex((call) => call[0] === CoderEventName.TaskTokenUsageUpdated)
		const taskAbortedIndex = calls.findIndex((call) => call[0] === CoderEventName.TaskAborted)
