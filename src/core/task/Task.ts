	...
	public async recursivelyMakeClineRequests(
		...
		while (stack.length > 0) {
			...
			try {
				...
				const hasToolUsesInContent = hasToolUses(this.assistantMessageContent)
				...
				if (hasTextContent || hasToolUsesInContent) {
					...
					const isolationResult = enforceNewTaskIsolation(assistantContent, this.assistantMessageContent)
					assistantContent = isolationResult.truncatedAssistantContent
					this.assistantMessageContent = isolationResult.truncatedAssistantMessageContent

					// Pre-inject error tool_results for truncated tools
					for (const errorResult of isolationResult.errorToolResults) {
						this.pushToolResultToUserContent(errorResult)
					}
				...
				if (hasTextContent || hasToolUsesInContent) {
					...
					const didToolUse = hasToolUses(this.assistantMessageContent)
