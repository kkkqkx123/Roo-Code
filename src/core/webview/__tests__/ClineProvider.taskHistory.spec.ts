// pnpm --filter coder-roo test core/webview/__tests__/ClineProvider.taskHistory.spec.ts

import * as vscode from "vscode"
import type { HistoryItem, ExtensionMessage } from "@coder/types"
...
describe("ClineProvider Task History Synchronization", () => {
	...
	describe("broadcastTaskHistoryUpdate", () => {
		it("sends taskHistoryUpdated message with sorted history", async () => {
			...
			const sentHistory = call?.[0]?.taskHistory as HistoryItem[] | undefined
			expect(sentHistory).toBeDefined()
			if (sentHistory) {
