	...
	(klass, prefix) => {
		...
		describe(`${klass.name}#hasNestedGitRepositories`, () => {
			it("throws error when nested git repositories are detected during initialization", async () => {
				...
				await mainGit.init()
				await mainGit.addConfig("user.name", "Coder")
				await mainGit.addConfig("user.email", "support@coder.com")
				...
				await nestedGit.addConfig("user.name", "Coder")
				await nestedGit.addConfig("user.email", "support@coder.com")
