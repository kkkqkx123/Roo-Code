...
describe("CodeIndexOrchestrator - stopIndexing", () => {
	...
	it("should reset _isProcessing flag after stopIndexing", async () => {
		// Make scanner hang until aborted
		scanner.scanDirectory.mockImplementation(
			async (_dir: string, _onError?: any, _onBlocksIndexed?: any, _onFileParsed?: any, signal?: AbortSignal) => {
				// Wait for abort signal
		...
		// Try to start indexing again - should succeed because _isProcessing was reset
		scanner.scanDirectory.mockResolvedValue({ stats: { processed: 1, skipped: 0 }, totalBlockCount: 1 })
		await orchestrator.startIndexing()

		// Verify that startIndexing was called twice (once before stop, once after)
