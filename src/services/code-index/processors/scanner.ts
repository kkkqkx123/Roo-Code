...
import { CodeBlock, ICodeParser, IEmbedder, IVectorStore, IDirectoryScanner, PointStruct } from "../interfaces"
...
export class DirectoryScanner implements IDirectoryScanner {
	...
	public async scanDirectory(
		...
		// Initialize RooIgnoreController if not provided (with both ignore rules enabled)
		...
		while (attempts < MAX_BATCH_RETRIES && !success) {
			...
			try {
				...
				const points = batchBlocks
					...
					.filter((point) => point.vector !== undefined) as PointStruct[]
