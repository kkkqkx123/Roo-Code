...
export class CodeParser implements ICodeParser {
	...
	private _chunkTextByLines(
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (lineLength > effectiveMaxChars) {
				...
				while (remainingLineContent && remainingLineContent.length > 0) {
			...
			if (currentChunkLength > 0 && currentChunkLength + lineLength > effectiveMaxChars) {
				...
				for (let j = i; j < lines.length; j++) {
					...
					if (currentLine) {
						remainderLength += currentLine.length + (j < lines.length - 1 ? 1 : 0)
					}
				}
	...
	private parseMarkdownContent(
		...
		if (markdownCaptures.length > 0 && markdownCaptures[0]?.node) {
		...
		for (let i = 0; i < markdownCaptures.length; i += 2) {
			...
			if (!definitionCapture || !nameCapture) continue
			...
			const headerLevel = headerMatch ? parseInt(headerMatch[1] || "1") : 1
