...
import { RooIgnoreController, IgnoreMode } from "../../../core/ignore/RooIgnoreController"
...
export class FileWatcher implements IFileWatcher {
	...
	constructor(
		...
		this.ignoreController = ignoreController || new RooIgnoreController(workspacePath, IgnoreMode.Both)
		if (ignoreInstance) {
					...
					while (retryCount < MAX_BATCH_RETRIES) {
						try {
							...
							if (retryCount === MAX_BATCH_RETRIES) {
								throw new Error(
									`Failed to upsert batch after ${MAX_BATCH_RETRIES} retries: ${upsertError.message}`, { cause: error },
								)
							}
		...
		try {
			...
			if (this.embedder && blocks.length > 0) {
				...
				pointsToUpsert = blocks
					...
					.filter((point): point is PointStruct => point !== null)
			}
