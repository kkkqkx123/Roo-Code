...
export function parseMarkdown(content: string): QueryCapture[] {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (i > 0) {
			...
			// Check for H1 (======)
			if (setextH1Regex.test(line) && validSetextTextRegex.test(prevLine)) {
			...
			// Check for H2 (------)
			if (setextH2Regex.test(line) && validSetextTextRegex.test(prevLine)) {
				const text = prevLine.trim()
	...
	for (let i = 0; i < captures.length; i += 2) {
		if (i + 1 < captures.length) {
			const capture1 = captures[i]
			const capture2 = captures[i + 1]
			if (capture1 && capture2) {
				headerCaptures.push([capture1, capture2])
			}
		} else {
			const capture = captures[i]
			if (capture) {
				headerCaptures.push([capture])
			}
		}
	}
	...
	for (let i = 0; i < headerCaptures.length; i++) {
		...
		if (!headerPair) continue

		if (i < headerCaptures.length - 1) {
			// End position is the start of the next header minus 1
			const nextHeaderStartRow = headerCaptures[i + 1]?.[0]?.node.startPosition.row ?? 0
