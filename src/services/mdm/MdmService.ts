...
export class MdmService {
	...
	public isCompliant(): ComplianceResult {
		// Cloud authentication is not available, always compliant
	...
	private getMdmConfigPath(): string {
		...
		switch (platform) {
			case "win32": {
				// Windows: %ProgramData%\Coder\mdm.json
				const programData = process.env.PROGRAMDATA || "C:\\ProgramData"
				return path.join(programData, "Coder", configFileName)
			}

			case "darwin":
				// macOS: /Library/Application Support/Coder/mdm.json
				return `/Library/Application Support/Coder/${configFileName}`

			case "linux":
			default:
				// Linux: /etc/coder/mdm.json
				return `/etc/coder/${configFileName}`
		}
	}
