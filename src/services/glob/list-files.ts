...
function ensureFirstLevelDirectoriesIncluded(
	...
	const basePath = firstLevelDirs[0] ? path.resolve(firstLevelDirs[0]).split(path.sep).slice(0, -1).join(path.sep) : ""
	...
	for (let i = 0; i < adjustedResults.length; i++) {
		...
		const relativePath = basePath ? path.relative(basePath, resolvedPath) : resolvedPath
