...
export class SkillsManager {
	...
	private async loadSkillMetadata(
		...
		try {
			...
			// Parse enabledForPrompt from frontmatter (default: true/undefined means enabled)
			const enabledForPrompt = frontmatter.enabledForPrompt === false ? false : undefined
	...
	/**
	 * Get skills enabled for the current mode.
	 * Filters getSkillsForMode results by enabled state.
	 *
	 * @param currentMode - The current mode slug (e.g., 'code', 'architect')
	 * @param disabledSkills - Optional array of disabled skill names from settings
	 * @returns Array of enabled skills for the mode
	 */
	getEnabledSkillsForMode(currentMode: string, disabledSkills?: string[]): SkillMetadata[] {
		...
		// Check if all skills are disabled (special "*" value)
		if (disabledSkills?.includes("*")) {
			return []
		}

		return skills.filter((skill) => this.isSkillEnabled(skill, disabledSkills))
	}

	/**
	 * Check if a specific skill is enabled.
	 * Checks multiple levels of configuration:
	 * 1. Global disabled list (disabledSkills)
	 * 2. Skill-level enabledForPrompt flag
	 *
	 * @param skill - The skill to check
	 * @param disabledSkills - Optional array of disabled skill names from settings
	 * @returns true if the skill is enabled, false otherwise
	 */
	isSkillEnabled(skill: SkillMetadata, disabledSkills?: string[]): boolean {
		// Check global disabled list
		if (disabledSkills?.includes(skill.name)) {
			return false
		}

		// Check skill-level enabledForPrompt flag
		// undefined or true means enabled, false means disabled
		if (skill.enabledForPrompt === false) {
			return false
		}
	 ...
	 *
	 * @param skillsEnabled - The global skills enabled setting
	 * @returns true if skills are enabled, false otherwise
	 */
