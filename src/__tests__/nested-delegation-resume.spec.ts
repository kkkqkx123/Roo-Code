...
describe("Nested delegation resume (A → B → C)", () => {
	...
	it("C completes → reopens B; then B completes → reopens A; emits correct events; no resume_task asks", async () => {
		...
		expect(eventNamesAfterC).toContain(CoderEventName.TaskDelegationCompleted)
		expect(eventNamesAfterC).toContain(CoderEventName.TaskDelegationResumed)
		...
		const completedEvents = emitSpy.mock.calls.filter(
			(c: any[]) => c[0] === CoderEventName.TaskDelegationCompleted,
		)
		const resumedEvents = emitSpy.mock.calls.filter((c: any[]) => c[0] === CoderEventName.TaskDelegationResumed)
