...
import { IpcServer } from "@coder/ipc"
...
export class API extends EventEmitter<CoderEvents> implements CoderAPI {
	...
	constructor(
		...
		if (enableLogging) {
			...
			this.logfile = path.join(os.tmpdir(), "coder-messages.log")
		} else {
			this.log = () => { }
		}
		...
		if (socketPath) {
			...
			ipc.on(IpcMessageType.TaskCommand, async (clientId, command) => {
				const sendResponse = (eventName: CoderEventName, payload: unknown[]) => {
					ipc.send(clientId, {
				...
				switch (command.commandName) {
					...
					case TaskCommandName.GetCommands:
						try {
							...
							sendResponse(CoderEventName.CommandsResponse, [
								commands.map((cmd) => ({
									name: cmd.name,
									source: cmd.source,
					...
					case TaskCommandName.GetModes:
						try {
							...
							sendResponse(CoderEventName.ModesResponse, [modes])
						} catch (error) {
							sendResponse(CoderEventName.ModesResponse, [[]])
						}

						break
					case TaskCommandName.GetModels:
						sendResponse(CoderEventName.ModelsResponse, [{}])
						break
	...
	public override emit<K extends keyof CoderEvents>(
		eventName: K,
		...args: K extends keyof CoderEvents ? CoderEvents[K] : never
	) {
		const data = { eventName: eventName as CoderEventName, payload: args } as TaskEvent
	...
	private async waitForWebviewLaunch(timeoutMs: number): Promise<boolean> {
		this.log(`[API#waitForWebviewLaunch] Starting to wait for webview launch, current viewLaunched state: ${this.sidebarProvider.viewLaunched}`)
		...
		try {
			await pWaitFor(() => {
				...
				if (elapsed % 500 === 0) {
					this.log(`[API#waitForWebviewLaunch] Still waiting... elapsed: ${elapsed}ms, viewLaunched: ${this.sidebarProvider.viewLaunched}`)
				}
				return this.sidebarProvider.viewLaunched
			}, {
				timeout: timeoutMs,
			...
			this.log(`[API#waitForWebviewLaunch] WebView launched successfully after ${Date.now() - startTime}ms`)
			return true
		} catch (error) {
			this.log(`[API#waitForWebviewLaunch] WebView did not launch within ${timeoutMs}ms, error: ${error instanceof Error ? error.message : String(error)}`)
			this.log(`[API#waitForWebviewLaunch] Final viewLaunched state: ${this.sidebarProvider.viewLaunched}`)
			return false
		}
	}

	private registerListeners(provider: ClineProvider) {
		provider.on(CoderEventName.TaskCreated, (task) => {
			// Task Lifecycle

			task.on(CoderEventName.TaskStarted, async () => {
				this.emit(CoderEventName.TaskStarted, task.taskId)
				await this.fileLog(`[${new Date().toISOString()}] taskStarted -> ${task.taskId}\n`)
			})

			task.on(CoderEventName.TaskCompleted, async (_, tokenUsage, toolUsage) => {
				this.emit(CoderEventName.TaskCompleted, task.taskId, tokenUsage, toolUsage, {
			...
			task.on(CoderEventName.TaskAborted, () => {
				this.emit(CoderEventName.TaskAborted, task.taskId)
			})

			task.on(CoderEventName.TaskFocused, () => {
				this.emit(CoderEventName.TaskFocused, task.taskId)
			})

			task.on(CoderEventName.TaskUnfocused, () => {
				this.emit(CoderEventName.TaskUnfocused, task.taskId)
			})

			task.on(CoderEventName.TaskActive, () => {
				this.emit(CoderEventName.TaskActive, task.taskId)
			})

			task.on(CoderEventName.TaskInteractive, () => {
				this.emit(CoderEventName.TaskInteractive, task.taskId)
			})

			task.on(CoderEventName.TaskResumable, () => {
				this.emit(CoderEventName.TaskResumable, task.taskId)
			})

			task.on(CoderEventName.TaskIdle, () => {
				this.emit(CoderEventName.TaskIdle, task.taskId)
			})

			// Subtask Lifecycle

			task.on(CoderEventName.TaskPaused, () => {
				this.emit(CoderEventName.TaskPaused, task.taskId)
			})

			task.on(CoderEventName.TaskUnpaused, () => {
				this.emit(CoderEventName.TaskUnpaused, task.taskId)
			})

			task.on(CoderEventName.TaskSpawned, (childTaskId) => {
				this.emit(CoderEventName.TaskSpawned, task.taskId, childTaskId)
			})

			task.on(CoderEventName.TaskDelegated as any, (childTaskId: string) => {
				; (this.emit as any)(CoderEventName.TaskDelegated, task.taskId, childTaskId)
			})

			task.on(CoderEventName.TaskDelegationCompleted as any, (childTaskId: string, summary: string) => {
				; (this.emit as any)(CoderEventName.TaskDelegationCompleted, task.taskId, childTaskId, summary)
			})

			task.on(CoderEventName.TaskDelegationResumed as any, (childTaskId: string) => {
				; (this.emit as any)(CoderEventName.TaskDelegationResumed, task.taskId, childTaskId)
			})

			// Task Execution

			task.on(CoderEventName.Message, async (message) => {
				this.emit(CoderEventName.Message, { taskId: task.taskId, ...message })
			...
			task.on(CoderEventName.TaskModeSwitched, (taskId, mode) => {
				this.emit(CoderEventName.TaskModeSwitched, taskId, mode)
			})

			task.on(CoderEventName.TaskAskResponded, () => {
				this.emit(CoderEventName.TaskAskResponded, task.taskId)
			})

			task.on(CoderEventName.QueuedMessagesUpdated, (taskId, messages) => {
				this.emit(CoderEventName.QueuedMessagesUpdated, taskId, messages)
			})

			// Task Analytics

			task.on(CoderEventName.TaskToolFailed, (taskId, tool, error) => {
				this.emit(CoderEventName.TaskToolFailed, taskId, tool, error)
			})

			task.on(CoderEventName.TaskTokenUsageUpdated, (_, tokenUsage, toolUsage) => {
				this.emit(CoderEventName.TaskTokenUsageUpdated, task.taskId, tokenUsage, toolUsage)
			})

			// Let's go!

			this.emit(CoderEventName.TaskCreated, task.taskId)
		})
	}
	...
	public getConfiguration(): CoderSettings {
		return Object.fromEntries(
			Object.entries(this.sidebarProvider.getValues()).filter(([key]) => !isSecretStateKey(key)),
		)
	}

	public async setConfiguration(values: CoderSettings) {
