...
export default defineConfig(({ mode }) => {
	...
	const define: Record<string, any> = {
		...
		"process.env.PKG_OUTPUT_CHANNEL": JSON.stringify("coder"),
		...(gitSha ? { "process.env.PKG_SHA": JSON.stringify(gitSha) } : {}),
	}

	// TODO: We can use the build utilities from src/build to generate `define` once the
	...
	if (mode === "nightly") {
		...
		define["process.env.PKG_OUTPUT_CHANNEL"] = JSON.stringify("coder-Nightly")
	}
