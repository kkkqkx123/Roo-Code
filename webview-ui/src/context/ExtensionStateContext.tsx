...
export const ExtensionStateContextProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
	const [state, setState] = useState<ExtensionState>({
		...
		cwd: "",
		ignoreMode: "both", // Default to using both .gitignore and .rooignore
	...
	const handleMessage = useCallback(
		(event: MessageEvent) => {
			...
			console.log('[ExtensionStateContext] Received message:', message.type)
			switch (message.type) {
				case "state": {
					...
					// Update skillsEnabled if present in state message
					if (newState.skillsEnabled !== undefined) {
						setSkillsEnabled(newState.skillsEnabled)
					}
					// Update disabledSkills if present in state message
					if (newState.disabledSkills !== undefined) {
						setDisabledSkills(newState.disabledSkills)
					}
					break
				}
				case "action": {
	...
	const contextValue: ExtensionStateContextType = {
		...
		setIgnoreMode: (value) => setState((prevState) => ({ ...prevState, ignoreMode: value })),
