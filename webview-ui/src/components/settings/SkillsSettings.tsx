...
import { Plus, Globe, Folder, Edit, Trash2, Settings, CheckSquare, Square } from "lucide-react"
import { Trans } from "react-i18next"

import type { SkillMetadata } from "@coder/types"
...
export const SkillsSettings: React.FC = () => {
	...
	// Handle global skills enabled toggle
	const handleSkillsEnabledToggle = useCallback(() => {
		const newValue = !(skillsEnabled ?? true)
		setSkillsEnabled(newValue)
		vscode.postMessage({
			type: "skillsEnabled",
			skillsEnabled: newValue,
		})
	}, [setSkillsEnabled, skillsEnabled])

	// Handle individual skill enable/disable toggle
	const handleSkillToggle = useCallback((skillName: string, isChecked: boolean) => {
		vscode.postMessage({
	...
	// Handle disable all skills
	const handleDisableAll = useCallback(() => {
		const allSkillNames = skills.map((s) => s.name)
		setDisabledSkills(allSkillNames)
		vscode.postMessage({
			type: "disabledSkills",
			disabledSkills: allSkillNames,
		})
	}, [skills, setDisabledSkills])
	...
	const renderSkillItem = useCallback(
		(skill: SkillMetadata) => {
			const isDisabled = !skillsEnabled || disabledSkills?.includes(skill.name)
			const isDisabledByMetadata = skill.enabledForPrompt === false

			return (
				<div
					key={`${skill.source}-${skill.name}-${skill.modeSlugs?.join(",") || "any"}`}
					className={`p-2.5 px-2 rounded-xl border ${isDisabled ? "bg-vscode-list-disabledBackground border-vscode-list-disabledBackground" : "border-transparent"}`}>
					<div className="flex items-start justify-between gap-2 flex-col min-[400px]:flex-row overflow-hidden">
						<div className="flex items-start gap-3 flex-1 min-w-0">
							{/* Enable/Disable Checkbox */}
							<StandardTooltip
								content={
									isDisabled ? t("settings:skills.enableSkill") : t("settings:skills.disableSkill")
								}>
								<Checkbox
									checked={!isDisabled}
									onCheckedChange={(checked) => handleSkillToggle(skill.name, checked === true)}
							...
							<div className="flex-1 min-w-0">
								{/* Skill name */}
								<div
									className={`flex items-center gap-2 overflow-hidden ${isDisabled ? "text-vscode-descriptionForeground" : ""}`}>
									<span className="font-medium truncate">{skill.name}</span>
									{isDisabledByMetadata && (
										<StandardTooltip content={t("settings:skills.disabledByMetadata")}>
								...
								{skill.description && (
									<div
										className={`text-xs text-vscode-descriptionForeground mt-1 line-clamp-3 ${isDisabled ? "opacity-70" : ""}`}>
										{skill.description}
									</div>
								)}
							</div>
		...
		[t, skillsEnabled, disabledSkills, handleOpenModeDialog, handleEditClick, handleDeleteClick, handleSkillToggle],
	)
		...
		<div className="flex flex-col h-full overflow-hidden">
			...
			<div className="flex-shrink-0">
				...
				<div className="flex flex-col gap-2 px-5 py-2">
					...
					{/* Global Skills Enabled Toggle */}
					<div className="flex items-center justify-between p-3 rounded-lg border border-vscode-panel-border">
						<div className="flex-1">
							<div className="font-medium">{t("settings:skills.enableSkills")}</div>
							<div className="text-xs text-vscode-descriptionForeground">
								{t("settings:skills.enableSkillsDescription")}
							</div>
						</div>
						<ToggleSwitch checked={skillsEnabled ?? true} onChange={handleSkillsEnabledToggle} />
					</div>

					{/* Bulk Operations - Only show when skills are enabled */}
					{skillsEnabled && skills.length > 0 && (
						<div className="flex items-center gap-2">
							<Button variant="secondary" size="sm" onClick={handleEnableAll} className="flex-1">
								<CheckSquare className="size-4 mr-1" />
								{t("settings:skills.enableAll")}
							</Button>
							<Button variant="secondary" size="sm" onClick={handleDisableAll} className="flex-1">
					...
					{/* Disabled Skills Count */}
					{skillsEnabled && (
						<div className="text-sm text-vscode-descriptionForeground">
							{disabledSkills?.length || 0} of {skills.length} skills disabled
						</div>
					)}
