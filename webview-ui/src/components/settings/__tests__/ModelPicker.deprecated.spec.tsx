...
vi.mock("@/components/ui/hooks/useSelectedModel", () => ({
	useSelectedModel: (apiConfiguration: any) => {
		const modelId = apiConfiguration?.openAiModelId || apiConfiguration?.openRouterModelId || "model-1"
...
describe("ModelPicker - Deprecated Models", () => {
		...
		render(
			<QueryClientProvider client={queryClient}>
				<ModelPicker
					...
					apiConfiguration={{
						apiProvider: "openai",
						openAiModelId: "deprecated-model",
					}}
					setApiConfigurationField={mockSetApiConfigurationField}
				/>
			</QueryClientProvider>,
	...
	it("should allow selecting non-deprecated models", async () => {
		...
		expect(mockSetApiConfigurationField).toHaveBeenCalledWith("openAiModelId", "model-2")
	})
