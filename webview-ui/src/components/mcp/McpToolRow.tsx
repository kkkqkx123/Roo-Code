...
import type { McpTool } from "@coder/types"
		...
		<div key={tool.name} className="py-2 border-b border-vscode-panel-border last:border-b-0">
			<div
				...
				<div className="flex items-center min-w-0 flex-1">
					<span
						className={`codicon codicon-symbol-method mr-2 flex-shrink-0 ${isToolEnabled
								? "text-vscode-symbolIcon-methodForeground"
								: "text-vscode-descriptionForeground opacity-60"
							}`}></span>
					<StandardTooltip content={tool.name}>
						<span
							className={`font-medium truncate ${isToolEnabled
			...
			{tool.description && (
				<div
					className={`mt-1 text-xs text-vscode-descriptionForeground ${isToolEnabled ? "opacity-80" : "opacity-40"
						}`}>
					{tool.description}
				</div>
			)}
